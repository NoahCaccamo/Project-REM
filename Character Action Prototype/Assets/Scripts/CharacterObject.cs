...
public class CharacterObject : MonoBehaviour
{
    ...
    // temp
    public int[] ExcessMemories = new int[5];
    ...
    void Start()
    {
        ...
        for (int i = 0; i < ExcessMemories.Length; i++)
        {
            ExcessMemories[i] = 1;
        }
    }
    ...
    void ExcessMemory()
    {
        if (ExcessMemories[0] != 0)
        {
            // store the state in the memory class
            StartState(13);

            //shift abilities
            for (int i = 0; i < ExcessMemories.Length - 1; i++)
            {
                ExcessMemories[i] = ExcessMemories[i + 1];
            }
            ExcessMemories[ExcessMemories.Length - 1] = 0;
        }
    }
