Shader "Custom/CharizardFlame2"
{
    Properties
    {
        ...
        _NoiseScale ("Noise Scale", Float) = 50.0       // Base scale/frequency of the noise applied to displacement (increased default)
        ...
        _Spikiness ("Spikiness", Range(1.0, 10.0)) = 5.0 // Controls how sharp and pointy the flame tips are (increased default)
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float _BaseDisplacement; // Declared new property
            ...
            v2f vert (appdata v)
            {
                ...
                // Remap noise from [-1, 1] to [0, 1] for outward-only displacement
                float remappedNoise = rawNoise * 0.5 + 0.5;

                // Apply spikiness: pow emphasizes peaks, making tips sharper.
                // saturate ensures the value stays within [0,1] after the power,
                // preventing extreme displacements.
                ...
                // _BaseDisplacement provides a consistent outward push (the "porcupine" base).
                // spikyNoise then scales the *difference* between _MaxDisplacement and _BaseDisplacement,
                // creating the varying spike heights on top of the base.
                float finalDisplacement = _BaseDisplacement + (spikyNoise * (_MaxDisplacement - _BaseDisplacement));

                // Incorporate flicker, applying it to the overall displacement magnitude
                finalDisplacement = finalDisplacement * (1.0 + _FlickerIntensity * flickerFactor);

                // Ensure final displacement is always positive (no inward displacement)
