    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float noise(float3 x)
            {
                float3 p = floor(x);
                float3 f = frac(x);
                // Changed from cubic interpolation (f * f * (3.0 - 2.0 * f)) to linear interpolation (f)
                // This makes the noise transitions sharper and more angular, creating a chunky, low-poly look.
                // f = f * f * (3.0 - 2.0 * f); // Original cubic interpolation
                // New: Linear interpolation for chunky look
                f = f;
